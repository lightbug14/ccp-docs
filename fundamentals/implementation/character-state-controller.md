...
This is an important concept to understand. A transition is evaluated in two places, in the `from state''\footnote{State where the transition originates.} and in the`to state''\footnote{State where the transition ends.}. These transitions are called Exit transition and Enter transition.

\parbox{0.9\linewidth} { \paragraph{ Exit transition } This is the part of the transition that is evaluated first, when trying to exit the current state \(hence its name\). }

\parbox{0.9\linewidth} { \paragraph{ Enter transition } This is the part of the transition that is evaluated secondly, when trying to enter the next state \(hence its name\).\ }
...
\parbox{0.9\linewidth} { \paragraph{Example} Let's say you want to implement a transition to a \textit{JetPack} state.

```text
In the \textit{Normal} state, to activate the jet pack only a specific button press is needed. This state doesn't have any information about the \textit{JetPack} state whatsoever (and it shouldn't). For the \textit{JetPack} state is another story. 


The \textit{JetPack} state must check if everything is ok, before allowing the transition to happen. For instance, if the character jet pack doesn't have any fuel, this should not be active. In the transition code inside the \textit{JetPack} state we can make sure that the fuel is enough.
```
