...
The character is modeled as an upright capsule. This means that the capsule height vector\footnote{Vector defined from the bottom sphere to the top sphere} and the up vector of the character transform will have always the same direction vector \(see figure \ref{fig:capsuleUpright}\).

\begin{figure} \centering \includegraphics\[width=0.2\linewidth\]{Image/capsuleUpright.png} \caption{} \label{fig:capsuleUpright} \end{figure}
...
\begin{equation} Width = 2 \times Collider.Radius \end{equation}

\begin{equation} Height = Collider.Height \end{equation}

The Width and Height values are represented using via the \`\`body size'' \(a \textit{Vector2}\). The figure \ref{fig:tutocharacter} shows the capsule width and height.

\begin{figure} \centering \includegraphics\[width=0.4\linewidth\]{Image/capsuleBodyDimensions.png} \caption{The character body.} \label{fig:tutocharacter} \end{figure}

%\vspace{0.5cm} %\mbox %{ % \underline{Note:} In play mode the collider offset will be automatically adjusted. %}

%\subsection{Play mode collider} % %The collider is used to define the BodySize vector, but it is also necessary for interact with rigidbodies. Once the character enters play mode you the dimensions of the collider are changed, especially the bottom of the capsule. This change is not so significant, but is important to avoid grounding and rigidbodies interaction problems. In play mode the capsule collider should look like figure \ref{fig:Tuto\_character\_Col}. % %\begin{figure} % \centering % \includegraphics\[width=0.45\linewidth\]{Image/Tuto\_character\_Col} % \caption{The capsule collider in play mode.} % \label{fig:Tuto\_character\_Col} %\end{figure}

\subsection{Scale}

This is an important topic, the \textit{CharacterBody} component defines the character size using an absolute value based on a \textit{Vector2} field. The \textit{CharacterActor} component reads this value in order to perform all the necessary physics queries \(mainly for collision detection\).

\textbf{So, What happens if the \textit{localScale} is not \textit{Vector3.one}?} Even though the collider will be scaled just fine, the internal \`\`physics size'' will not. So, it is not recommend to scale the character using the \textit{Transform} component.
